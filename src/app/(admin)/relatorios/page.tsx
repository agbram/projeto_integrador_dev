...
const openOrdersPage = (order: Order) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const processedData = processAnalyticsData(ordersData, customersData, productsData);
  ...
  const processAnalyticsData = (
    ...
    const topCustomerEntry = Object.entries(customerOrderCount).reduce((top, [customerId, data]) => {
      if (!top || data.count > top.data.count) {
        const customer = customers.find(c => c.id === parseInt(customerId));
        return { customer, data };
      }
      return top;
    }, null as { customer: Customer | undefined; data: { count: number; total: number } } | null);
    ...
    const leastSoldProductEntry = productEntries.reduce((least, [productId, data]) => {
    ...
    const recentOrders = filteredOrders
      .sort((a, b) => new Date(b.orderDate
          ? new Date(b.orderDate.toString()).toISOString() : "").getTime() - new Date(a.orderDate
          ? new Date(a.orderDate.toString()).toISOString(): "").getTime())
      .slice(0, 5);
  ...
  const filterOrdersByTimeRange = (orders: Order[], range: "week" | "month" | "year"): Order[] => {
    ...
    return orders.filter(order => new Date(order.orderDate
          ? new Date(order.orderDate.toString()).toISOString() : "") >= startDate);
  };
            ...
            <div className={styles.customerInfo}>
              ...
              <div className={styles.customerStats}>
                ...
                <div className={styles.statItem}>
                  ...
                  <strong>{formatCurrency(analyticsData.topCustomer.totalSpent)}</strong>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className={styles.productInfo}>
              ...
              <div className={styles.productStats}>
                <div className={styles.statItem}>
                  ...
                  <strong>{analyticsData.topProduct.quantitySold} unidades</strong>
                </div>
                <div className={styles.statItem}>
                  <span>Receita:</span>
                  <strong>{formatCurrency(analyticsData.topProduct.revenue)}</strong>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={styles.analysisContent}>
            <div className={styles.productInfo}>
              ...
              <div className={styles.productStats}>
                <div className={styles.statItem}>
                  <span>Quantidade:</span>
                  <strong>{analyticsData.leastSoldProduct.quantitySold} unidades</strong>
                </div>
                <div className={styles.statItem}>
                  <span>Receita:</span>
                  <strong>{formatCurrency(analyticsData.leastSoldProduct.revenue)}</strong>
                </div>
              </div>
            </div>
          </div>
